   ...
   <div class="col-12">
      ...
      <div class="widget-content widget-content-area">
         ...
         @include('layouts.alert.alert')
            <form class="form-inline mt-4 mb-1">
               <h4 class="col-10"><b>{{$pageName}}</b></h4>
                  <div class="col-auto">
                     <a href="{{ route('ciritipe.create') }}" class="btn btn-primary mb-2"><i data-feather="plus"></i>{{$pageActive}}</a>
                  </div>
            </form>
         </div> {{-- widget-header --}}
         <div class="table-responsive mb-4 style-2">
            <table id="style-2" class="table style-2 table-hover non-hover">
               ...
               <tbody>
                  @foreach ($ciritipe as $ciri)
                  <tr>
                     <td scope="row">{{$loop->iteration}}</td>
                     <td>{{$ciri->tipekepribadian->namatipe}}</td>                        
                     <td>{{Str::limit($ciri->ciri_kepribadian, 30, '...')}}</td>
                     <td class="text-center">
                        <div class="dropdown custom-dropdown">
                           ...
                           <div class="dropdown-menu" aria-labelledby="dropdownMenuLink12">
                              ...
                              <a class="dropdown-item btn btn-sm text-warning" href="{{ route('ciritipe.edit', $ciri->id) }}">Edit</a>
                              <a class="dropdown-item btn btn-sm text-danger" href="#" data-toggle="modal" data-target="#delete" data-id="{{ $ciri->id }}">Delete</a>
                           </div>
                        </div>
                     </td>
                  </tr>
      ...
      <div class="modal fade" id="delete" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
         <div class="modal-dialog modal-dialog-centered" role="document">
            <form action="{{ route('ciritipe.destroy', $ciri->id) }}" method="post">
               @csrf
               @method('DELETE')
